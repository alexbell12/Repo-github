...
@section('content')
<div class="mb-4">
    <a href="{{ route('events.index') }}" class="text-indigo-600 hover:text-indigo-800 inline-flex items-center gap-2 text-sm">
...
<div class="card-glass rounded-2xl p-8 max-w-4xl mx-auto">
    <h1 class="text-3xl font-bold mb-2 text-gray-900">{{ $event->title }}</h1>
    <span class="inline-flex items-center gap-1 px-3 py-1 rounded-full bg-green-100 text-green-800 text-sm">
    ...
    <div class="grid md:grid-cols-2 gap-8 mt-8">
        <div>
            <h2 class="text-lg font-semibold mb-4 text-gray-900">Informasi Event</h2>
        ...
        <div>
            <h2 class="text-lg font-semibold mb-4 text-gray-900">Deskripsi Event</h2>
            <p class="text-gray-700">{{ $event->description ?: 'Tidak ada deskripsi tersedia untuk event ini.' }}</p>
        </div>
    </div>
        ...
        @auth
            ...
            @if($reg)
                @if($reg->isVerified())
                    ...
                    @if($reg->hasAttended())
                        <a href="{{ route('events.certificate', $event->slug) }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-white border border-indigo-200 text-indigo-700 font-semibold hover:bg-indigo-50">
            ...
            @endif
        @else
            <a href="{{ route('login') }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-gray-100 hover:bg-gray-200 text-gray-800 font-medium">Login untuk mendaftar</a>
            <a href="{{ route('events.register.form', $event->slug) }}" class="inline-flex items-center gap-2 px-5 py-2.5 rounded-lg bg-indigo-600 text-white font-semibold hover:bg-indigo-700">Daftar sebagai tamu</a>
        @endauth
    </div>
</div>
@endsection
