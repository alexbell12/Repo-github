...
<div class="max-w-2xl mx-auto">
    <div class="card-glass rounded-2xl p-8">
        ...
        <p class="text-gray-600 text-sm mb-6">Isi data event di bawah ini</p>
        ...
        <form method="POST" action="{{ route('admin.events.store') }}" class="space-y-4">
            ...
            <div>
                ...
                <textarea name="description" rows="4" placeholder="Masukkan deskripsi event" class="form-input">{{ old('description') }}</textarea>
            </div>
            <div class="grid md:grid-cols-2 gap-4">
                <div>
                    <label class="block text-sm font-medium mb-1 text-gray-700">Waktu Mulai *</label>
                    <input type="datetime-local" name="start_time" value="{{ old('start_time') }}" required class="form-input">
                </div>
                <div>
                    <label class="block text-sm font-medium mb-1 text-gray-700">Waktu Selesai *</label>
                    <input type="datetime-local" name="end_time" value="{{ old('end_time') }}" required class="form-input">
                </div>
            </div>
            <div>
                <label class="block text-sm font-medium mb-1 text-gray-700">Lokasi Event</label>
                <input type="text" name="location" value="{{ old('location') }}" class="form-input" placeholder="Nama lokasi acara">
            </div>
            <div class="p-4 rounded-lg bg-gray-50 border border-gray-200 space-y-4">
                ...
                <div class="grid md:grid-cols-2 gap-4">
                    <div>
                        ...
                        <input type="number" step="any" name="latitude" id="latitude" value="{{ old('latitude') }}" class="form-input" placeholder="-0.502106">
                    </div>
                    <div>
                        <label class="block text-sm font-medium mb-1 text-gray-600">Longitude</label>
                        <input type="number" step="any" name="longitude" id="longitude" value="{{ old('longitude') }}" class="form-input" placeholder="101.447777">
                    </div>
                </div>
                <div>
                    <label class="block text-sm font-medium mb-1 text-gray-600">Radius (meter)</label>
                    <input type="number" name="location_radius" value="{{ old('location_radius', 100) }}" min="10" max="5000" class="form-input">
                </div>
